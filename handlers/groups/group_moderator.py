...
@dp.message_handler(IsGroup(), Command("choqintoy", prefixes="@#$&!/"), AdminFilter())
async def ban_user(message: types.Message):
    ...
    await message.answer(f"Foydalanuvchi {message.reply_to_message.from_user.get_mention(as_html=True)} cho'qintoy deb e'lon qilindi!\n"
                         f"Sabab: \n<b>Qizlarga cho'qingani uchun!</b>")
    service_message = await message.reply("Xabar 5 sekunddan so'ng o'chib ketadi.")
